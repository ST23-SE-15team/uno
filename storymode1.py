...
def story_map1():
    ...
    Hsan_rect.center = (400, 300)

    EU_rect = EU_image.get_rect()
    EU_rect.center = (170, 240)
    ...
    while running:
        ...
        #defining EU_x, EU_y, EU_width, EU_height
        EU_x = 140
        EU_y = 190
        EU_width = 98
        EU_height = 85
        ...
        #캐릭터와 EU이 충돌하면 캐릭터의 위치가 멈추게 설정, EU위에 텍스트 출력
        if char_x + char_width > EU_x and char_x < EU_x + EU_width and char_y + char_height > EU_y and char_y < EU_y + EU_height:
            char_x -= move_x
            char_y -= move_y
            EU_text = font.render("Ihoteu: ENTER!", True, (0, 0, 0))
            screen.blit(EU_text, (EU_x, EU_y - 20))
